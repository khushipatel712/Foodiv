const Setup = require('../Models/Setup'); // Adjust the path as needed
...
exports.updateSetup = async (req, res) => {
    try {
        ...
        let existingData = await Setup.findOne();

        if (!existingData) {
            existingData = new Setup({ title: '', description: '', setup: [] });
        }
        ...
        res.status(200).json({ message: 'Setup updated successfully!', data: existingData });
    } catch (error) {
...
exports.removeSetupItem = async (req, res) => {
    try {
        ...
        const data = await Setup.findOneAndUpdate(
