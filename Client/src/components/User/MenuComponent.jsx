...
import { getDrawerDataFromDB } from './IndexdDBUtils';
...
const MenuComponent = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const drawerData = await getDrawerDataFromDB();
                if (drawerData) {
                    const { cartItems, totalAmount } = drawerData;
                    setCartItems(cartItems || []);
                    // Use additional data if needed
                }
            } catch (error) {
        ...
        <>
            <div className="flex flex-col lg:flex-row h-auto lg:mx-10  mx-1">
                ...
                <div className={`bg-white lg:w-1/6 flex text-right pt-10 lg:block ${isMenuOpen ? 'block' : 'hidden'} lg:static z-10 inset-0 lg:h-96 h-full shadow-lg lg:shadow-none`}>
                    ...
                    {uniqueCategories
                        .filter(categoryName => groupedItems[categoryName][0]?.category.show)
                ...
                <div className="flex-1 px-1 sm:px-7 md:px-10 lg:pl-10 m-10 lg:border-l-2 h-full">
                    ...
                    <div className="flex items-center sm:justify-between mb-6 space-x-2">
                        <div className="flex items-center border text-[12px] sm:text-base border-gray-300 rounded-md w-full lg:w-1/3">
                            ...
                            <label className="flex items-center cursor-pointer">
                                <div className="relative">
                                    ...
                                    >
                                        {isVegOnly ? (
                                            <>
                                                <span className="text-green-500 whitespace-nowrap text-[13px] sm:text-base">Veg Only</span>
                                                <FaToggleOn className="text-green-500 size-4 sm:size-6 ml-1" />
                                            </>
                                        ) : (
                                            <>
                                                <span className="text-gray-500 whitespace-nowrap text-[13px] sm:text-base">Veg Only</span>
                                            ...
                                            <div key={itemIdx}>
                                                ...
                                                <div className="mt-2 border-gray-300 rounded-xl p-3 shadow-lg flex justify-between items-center">
                                                    ...
                                                    {cartItem ? (
                                                        <div className="flex items-center ">
                                                            <button
                                                                className="sm:px-2 sm:py-1 px-1 bg-orange-500 text-white font-bold rounded-l"
                                                                onClick={() => handleDecrement(item._id)}
                                                            >
                                                                -
                                                            </button>
                                                            <span className="sm:px-[10px] px-1">{cartItem.quantity}</span>
                                                            <button
                                                                className="sm:px-2 sm:py-1 px-[3px] bg-orange-500 text-white font-bold  rounded-r"
            ...
            <button
                className="lg:hidden text-sm md:text-base fixed md:bottom-14 bottom-12 left-1/2 transform -translate-x-1/2 bg-black text-white px-[14px] py-[8px] border-4 border-white rounded-full shadow-2xl z-20"
                onClick={toggleMenu}
            >
                <div className='flex flex-wrap justify-center items-center font-[480]'>
