...
const ServeMoreDashboard = () => {
    const [serveMoreData, setServeMoreData] = useState({
        title: '',
        logo: '',
        image: '',
        description: '',
    ...
    const fetchServeMore = () => {
        axios.get('http://localhost:5001/api/servemore')
            .then((res) => {
                if (res.data) {
                    setServeMoreData(res.data);
    ...
    const handleInputChange = (e) => {
        ...
        setServeMoreData({ ...serveMoreData, [name]: value });
    };
    ...
    const handleLogoChange = (e) => {
        ...
        if (file) {
            ...
            setServeMoreData({ ...serveMoreData, logo: file });
        }
    };
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            setServeMoreData({ ...serveMoreData, image: file });
        }
    };
    ...
    const handleRemoveExistingKeypoint = async (keypoint) => {
        try {
            await axios.delete('http://localhost:5001/api/servemore/keypoints', {
                params: { keypoint } 
            });
    ...
    const handleSubmit = (e) => {
        ...
        formData.append('title', serveMoreData.title);
        formData.append('description', serveMoreData.description);
    
        if (serveMoreData.logo instanceof File) formData.append('logo', serveMoreData.logo);
        if (serveMoreData.image instanceof File) formData.append('image', serveMoreData.image);
    
        // Combine existing keypoints with new keypoints
        const allKeypoints = [...serveMoreData.keypoints, ...newKeypoints.filter(kp => kp.trim() !== '')];
        ...
        axios.put('http://localhost:5001/api/servemore', formData, {
            headers: {
        ...
        .then(() => {
            ...
            fetchServeMore(); // Refresh the data
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-md">
            <h2 className="text-2xl font-bold mb-6">ServeMore Dashboard</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mb-6">
                    ...
                    <textarea
                        name="description"
                        value={serveMoreData.description}
                        onChange={handleInputChange}
                        className="mt-1 block w-full p-2 border rounded-md"
                        required
                    />
                ...
                {serveMoreData.keypoints?.map((keypoint, index) => (
...
export default ServeMoreDashboard;
